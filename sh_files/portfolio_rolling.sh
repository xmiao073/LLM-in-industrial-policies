#!/bin/bash
#SBATCH --job-name=port_roll_dynamic
...
#SBATCH --output=/hpc2hdd/home/jliu043/policy/logs/rolling_%A_%a.out
#SBATCH --error=/hpc2hdd/home/jliu043/policy/logs/rolling_%A_%a.err

source ~/.bashrc
conda activate cnn_env
cd /hpc2hdd/home/jliu043/policy/scripts

# ---------- Parameter grid (rolling; dynamic only) ----------
...
# ---------- Output directory: rolling/p<period>/<ret>/<sign>/<wt> ----------
# 备注：脚本会在该目录下自动创建 dynamic / dynamic_{Xm} 子目录
BASE_DIR="/hpc2hdd/home/jliu043/policy/portfolio/rolling"
OUT_DIR="${BASE_DIR}/p${period}/${ret}/${sign}/${wt}"
mkdir -p "$OUT_DIR"

echo "[INFO] idx=$idx → dynamic / $ret / $sign / $wt / rolling / windows=[$WINDOWS_STR]"
echo "[INFO] outputs → $OUT_DIR"

for period in ${PERIOD_ARR[@]}; do
  OUT_DIR="${BASE_DIR}/p${period}/${ret}/${sign}/${wt}"
  mkdir -p "$OUT_DIR"
  echo "[INFO] running period=$period → $OUT_DIR"
